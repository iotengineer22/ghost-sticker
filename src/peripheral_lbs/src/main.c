...
/* オーバーレイの alias に合わせて LED を定義 */
static const struct gpio_dt_spec run_led = GPIO_DT_SPEC_GET(DT_ALIAS(led0), gpios);
static const struct gpio_dt_spec user_led = GPIO_DT_SPEC_GET(DT_ALIAS(led1), gpios);
...
{
	gpio_pin_set_dt(&user_led, led_state ? 1 : 0);
}
...
int main(void)
{
	...
	if (!gpio_is_ready_dt(&run_led)) {
		printk("Run LED not ready\n");
		return 0;
	}
	gpio_pin_configure_dt(&run_led, GPIO_OUTPUT_INACTIVE);
